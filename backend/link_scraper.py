...
def extract_image_urls_from_html(html: str, base_url: str) -> tuple[list[str], dict]:
            ...
            for item in items:
                ...
                if "product" in item_type or "itempage" in item_type or "offers" in item:
                    ...
                    if isinstance(img, str):
                        ...
                        candidates.append(_clean_url(img["url"], base_url))
        except Exception:
            pass
    ...
    # Product Title in e-commerce elements
    if not metadata.get("title"):
        title_elem = soup.find(id=re.compile(r"(productTitle|product-title|item-title)", re.I)) or soup.find("h1")
        if title_elem:
            metadata["title"] = title_elem.get_text(strip=True)

    # 3. Amazon specific image tags & dynamic images
    landing_img = soup.find("img", id=re.compile(r"(landingImage|imgBlkFront)", re.I))
    if landing_img:
        if landing_img.get("data-old-hires"):
            candidates.append(_clean_url(landing_img["data-old-hires"], base_url))
        if landing_img.get("data-a-dynamic-image"):
            try:
                dyn_map = json.loads(landing_img["data-a-dynamic-image"])
                # Sort by largest image area
                sorted_urls = sorted(
                    dyn_map.keys(),
                    key=lambda u: dyn_map[u][0] * dyn_map[u][1] if isinstance(dyn_map[u], list) and len(dyn_map[u]) >= 2 else 0,
                    reverse=True
                )
                for u in sorted_urls:
                    candidates.append(_clean_url(u, base_url))
            except Exception:
                pass
        if landing_img.get("src"):
            candidates.append(_clean_url(landing_img["src"], base_url))

    # 4. Generic product images and high-res data attributes
    for img in soup.find_all("img"):
        for attr in ["data-old-hires", "data-zoom-image", "data-large-img", "data-highres", "data-src", "src"]:
            val = img.get(attr)
            if not val:
                continue
            cleaned = _clean_url(val, base_url)
            # Filter out icons, spinners, base64, tracking pixels
            lower = cleaned.lower()
            if lower.startswith("data:image"):
                continue
            if any(skip in lower for skip in ["sprite", "pixel", "icon", "logo", "spinner", "badge", "rating", "star", ".svg"]):
                continue
    ...
    unique_candidates = []
    for c in candidates:
        if c and c not in seen and c.startswith("http"):
            seen.add(c)
...
async def fetch_product_image_and_metadata(url: str) -> tuple[bytes, str, dict]:
    """Fetch product image bytes, file extension, and metadata from a product link or direct image URL."""
    url = url.strip()
    ...
    async with httpx.AsyncClient(headers=HEADERS, follow_redirects=True, timeout=20.0) as client:
        ...
        # Parse HTML for product images & metadata
        html = resp.text
        ...
        # Download first valid candidate image
            ...
            try:
                ...
                if img_resp.status_code == 200 and len(img_resp.content) > 1500:
                    ...
                    if w >= 150 and h >= 150:
                        ...
                        return img_resp.content, ext, metadata
            except Exception as e:
                logger.warning("Failed downloading candidate image %s: %s", img_url, e)
                continue
